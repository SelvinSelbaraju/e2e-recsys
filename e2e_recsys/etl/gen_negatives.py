...
class NegativeSampler:
    ...
    def _get_candidate_dates(self) -> List[str]:
        start_date = datetime.strptime(
            self.data[self.date_col].min(), "%Y-%m-%d"
        )
        end_date = datetime.strptime(
            self.data[self.date_col].max(), "%Y-%m-%d"
        )
        ...
        while current_date <= end_date:
            self.candidate_dates = np.append(
                self.candidate_dates, current_date.strftime("%Y-%m-%d")
            )
            current_date += timedelta(days=1)

    def add_random_data(self):
        negatigves_df_dict = {
            "t_dat": np.random.choice(
                self.candidate_dates, size=self.num_negatives, replace=True
            ),
            "customer_id": np.random.choice(
                self.candidate_customers, size=self.num_negatives, replace=True
            ),
            "article_id": np.random.choice(
                self.candidate_articles, size=self.num_negatives, replace=True
            ),
        ...
        negatives = pd.DataFrame(
            negatigves_df_dict,
            index=range(len(self.data), len(self.data) + self.num_negatives),
        )
